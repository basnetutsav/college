...
# ======================
# TAB: Stats (MAIN TAB) — fewer graphs, all tables kept
# ======================
with tab_stats:
    st.subheader("Stats")
    ...
    def _download_table(df_out: pd.DataFrame, base_name: str):
        # CSV (always)
        try:
            st.download_button(
        ...
        try:
            ...
            st.download_button(
                ...
                key=pkey(f"dl_{base_name}_xlsx"),
            )
        except Exception:
            pass
    ...
    # Optional: fewer graphs switch (default OFF)
    show_charts = st.toggle("Show charts (optional)", value=False, key="stats_show_charts")
    st.caption("Tables always stay. Turn charts on only when you need visuals.")

    st.divider()

    # ======================================================
    # 1) Channel value differences (Kruskal) + summary table
    # ======================================================
    st.markdown("### 1) Do channels differ on value?")

    base = df_stat.dropna(subset=["Channel", metric]).copy()
    base = base[np.isfinite(base[metric])]

    if base.empty or base["Channel"].nunique() < 2:
        st.info("Not enough channel data under current filters.")
        p_val_kruskal = np.nan
        ch_summary = pd.DataFrame()
    else:
        # Summary table (ALWAYS)
        total_metric = float(base[metric].sum()) if len(base) else 0.0
        ch_summary = (
        ...
        ch_summary["share"] = np.where(total_metric > 0, ch_summary["total"] / total_metric, np.nan)
        ch_summary = ch_summary.sort_values("total", ascending=False).copy()

        ch_tbl = ch_summary.copy()
        ch_tbl["total"] = ch_tbl["total"].round(0)
        ch_tbl["mean"] = ch_tbl["mean"].round(2)
        ch_tbl["median"] = ch_tbl["median"].round(2)
        ch_tbl["share"] = ch_tbl["share"].round(4)

        ch_tbl = rank_df(ch_tbl)
        st.dataframe(
            ch_tbl.set_index("#")[["Channel", "orders", "total", "share", "mean", "median"]],
            use_container_width=True,
        )
        _download_table(ch_tbl.drop(columns=["#"]), "stats_channel_summary")

        # Kruskal test (text result)
        p_val_kruskal = np.nan
        if stats is None:
            st.info("SciPy not available in this environment (stat test disabled).")
        else:
            groups = (
                base.groupby("Channel")[metric]
                .apply(lambda s: pd.to_numeric(s, errors="coerce").dropna().values)
                .tolist()
            )
            if len(groups) >= 2 and all(len(g) > 1 for g in groups):
                _, p_val_kruskal = stats.kruskal(*groups)
                st.write(
                    f"**Kruskal–Wallis p-value:** **{_p_fmt(p_val_kruskal)}** → "
                    + ("Channels likely differ." if np.isfinite(p_val_kruskal) and p_val_kruskal < 0.05 else "No strong evidence of a difference.")
                )
            else:
                st.write("Not enough usable values per channel for the test.")

        # FEWER GRAPHS: only ONE chart (optional)
        if show_charts and not ch_summary.empty:
            fig1 = px.bar(
                ch_summary,
                x="Channel",
                y="total",
                title=f"Total {metric_name} by Channel",
            )
            fig1.update_layout(xaxis={"categoryorder": "total descending"})
            fig1.update_traces(hovertemplate="<b>%{x}</b><br>Total: %{y:$,.0f}<extra></extra>")
            fig1 = fig_tight(fig1, height=420)
            st.plotly_chart(fig1, use_container_width=True, key=pkey("stats_ch_total"))
            download_html(fig1, "stats_01_total_by_channel.html")

    # Dynamic insights (auto-updates with uploaded file)
    if not ch_summary.empty:
        top_ch = str(ch_summary.iloc[0]["Channel"])
        top_ch_share = float(ch_summary.iloc[0]["share"]) if "share" in ch_summary.columns and np.isfinite(ch_summary.iloc[0]["share"]) else np.nan
        insights_md = "\n".join([
            f"- Top channel by total is **{top_ch}**." + (f" (Share: **{top_ch_share:.1%}**)" if np.isfinite(top_ch_share) else ""),
            f"- Test result p-value: **{_p_fmt(p_val_kruskal)}**." if np.isfinite(p_val_kruskal) else "- Statistical test may be unavailable (or not enough data).",
        ])
    else:
        insights_md = "- Not enough channel data to summarize under current filters."

    why_md = "This tells you whether channel differences are likely real, so you don’t overreact to random variation."
    recs_md = "\n".join([
        "- If one channel dominates total value, prioritize inventory + marketing there.",
        "- If channels differ significantly, avoid using one global pricing/offer strategy across all channels.",
        "- Track changes after promotions to confirm impact (not just noise).",
    ])
    ...
    # ======================================================
    # 2) Country × Channel mix (chi-square) + mix tables
    # ======================================================
    st.markdown("### 2) Is channel mix different by country?")

    top_n_countries = st.slider("Top N countries for mix test", 3, 25, 10, key="stats_mix_top_n")
    df_cc = df_stat.dropna(subset=["Country", "Channel"]).copy()

    if df_cc.empty or df_cc["Country"].nunique() < 2 or df_cc["Channel"].nunique() < 2:
        st.info("Not enough data for a country × channel mix analysis under current filters.")
        p_val_chi2 = np.nan
    else:
        # Choose top countries by total metric (fallback to count if metric is mostly missing)
        tmp_metric = df_cc.dropna(subset=[metric]).copy()
        if tmp_metric.empty:
            top_countries = df_cc["Country"].value_counts().head(top_n_countries).index
        else:
            top_countries = tmp_metric.groupby("Country")[metric].sum().sort_values(ascending=False).head(top_n_countries).index

        df_top = df_cc[df_cc["Country"].isin(top_countries)].copy()

        # TABLE 1: counts pivot
        ct = pd.crosstab(df_top["Country"], df_top["Channel"])
        ct_tbl = ct.reset_index()
        st.markdown("**Order counts (Country × Channel)**")
        st.dataframe(ct_tbl, use_container_width=True)
        _download_table(ct_tbl, "stats_country_channel_counts")

        # TABLE 2: shares pivot
        shares = ct.div(ct.sum(axis=1).replace(0, np.nan), axis=0)
        sh_tbl = shares.reset_index()
        st.markdown("**Channel share within each country**")
        st.dataframe(sh_tbl, use_container_width=True)
        _download_table(sh_tbl, "stats_country_channel_shares")

        # Chi-square test (text)
        p_val_chi2 = np.nan
        if stats is None:
            st.info("SciPy not available in this environment (chi-square test disabled).")
        else:
            try:
                chi2, p_val_chi2, _, _ = stats.chi2_contingency(ct.values)
                st.write(
                    f"**Chi-square p-value:** **{_p_fmt(p_val_chi2)}** → "
                    + ("Mix likely differs by country." if np.isfinite(p_val_chi2) and p_val_chi2 < 0.05 else "No strong evidence the mix differs.")
                )
            except Exception:
                st.write("Could not run chi-square test (data shape/values issue).")

        # FEWER GRAPHS: optional heatmap only (off by default)
        if show_charts and not shares.empty:
            fig2 = px.imshow(
                shares,
                aspect="auto",
                title="Heatmap: Channel Share by Country",
                labels=dict(x="Channel", y="Country", color="Share"),
            )
            fig2 = fig_tight(fig2, height=460)
            st.plotly_chart(fig2, use_container_width=True, key=pkey("stats_mix_hm"))
            download_html(fig2, "stats_02_channel_share_heatmap.html")

    # Dynamic insights
    insights_md = "\n".join([
        f"- Mix test p-value: **{_p_fmt(p_val_chi2)}**." if np.isfinite(p_val_chi2) else "- Mix test may be unavailable (or not enough data).",
        "- Use the share table to see which channel wins inside each country.",
    ])
    why_md = "This prevents a one-size-fits-all channel plan and helps you tailor strategy per market."
    recs_md = "\n".join([
        "- For each top country, back the 1–2 strongest channels and build a repeatable playbook.",
        "- Countries with a split mix are good candidates for A/B testing channel investments.",
        "- If one channel dominates a country, fix bottlenecks there first (stock, shipping, promo timing).",
    ])
    ...
    # ======================================================
    # 3) Shipping lag relationship (correlation) + lag tables
    # ======================================================
    st.markdown("### 3) Does shipping lag relate to performance?")
    ...
    if lag_col is None or lag_col not in df_stat.columns:
        ...
        lag_df = df_stat.dropna(subset=[lag_col, metric]).copy()
        lag_df["Ship Lag (days)"] = pd.to_numeric(lag_df[lag_col], errors="coerce")
        lag_df = lag_df[np.isfinite(lag_df["Ship Lag (days)"]) & np.isfinite(lag_df[metric])]

        if lag_df.empty:
            st.info("Not enough usable lag + metric values under current filters.")
            p_val_corr = np.nan
            corr_val = np.nan
        else:
            # TABLE: lag summary by channel (ALWAYS)
            lag_sum = (
                lag_df.groupby("Channel", as_index=False)
                .agg(
                    orders=("Ship Lag (days)", "count"),
                    avg_lag=("Ship Lag (days)", "mean"),
                    med_lag=("Ship Lag (days)", "median"),
                    total_metric=(metric, "sum"),
                )
                .sort_values("avg_lag", ascending=False)
            )
            lag_sum["avg_lag"] = lag_sum["avg_lag"].round(1)
            lag_sum["med_lag"] = lag_sum["med_lag"].round(1)
            lag_sum["total_metric"] = lag_sum["total_metric"].round(0)

            st.markdown("**Lag summary by channel**")
            st.dataframe(lag_sum, use_container_width=True)
            _download_table(lag_sum, "stats_lag_summary_by_channel")

            # Correlation (text)
            corr_val = np.nan
            p_val_corr = np.nan
            if stats is None:
                st.info("SciPy not available in this environment (correlation disabled).")
            else:
                try:
                    corr_val, p_val_corr = stats.spearmanr(lag_df["Ship Lag (days)"], lag_df[metric], nan_policy="omit")
                    st.write(
                        f"**Spearman correlation:** **{corr_val:.3f}** (p={_p_fmt(p_val_corr)})"
                    )
                except Exception:
                    st.write("Could not compute correlation for lag vs metric.")

            # FEWER GRAPHS: optional scatter only (off by default)
            if show_charts:
                fig3 = px.scatter(
                    lag_df,
                    x="Ship Lag (days)",
                    y=metric,
                    title=f"{metric_name} vs Shipping Lag",
                    trendline="ols" if "statsmodels" in globals() else None,
                )
                fig3 = fig_tight(fig3, height=460)
                st.plotly_chart(fig3, use_container_width=True, key=pkey("stats_lag_scatter"))
                download_html(fig3, "stats_03_metric_vs_lag.html")

    # Dynamic insights
    if np.isfinite(corr_val):
        direction = "decreases" if corr_val < 0 else "increases"
        insights_md = "\n".join([
            f"- As shipping lag goes up, the metric tends to **{direction}** (corr={corr_val:.3f}).",
            f"- p-value: **{_p_fmt(p_val_corr)}**.",
        ])
    else:
        insights_md = "- Shipping lag relationship not available (missing lag, SciPy unavailable, or not enough data)."

    why_md = "If lag is hurting results, fixing delivery speed can directly improve performance."
    recs_md = "\n".join([
        "- Prioritize fixing lag where you also have meaningful volume (high total metric + high lag).",
        "- Clean negative/invalid lag rows to keep lag KPIs reliable.",
        "- Set SLAs per channel/country and re-check this section after changes.",
    ])
    insights_expander("Shipping Lag Impact", insights_md, why_md, recs_md)
