...
with tab_overview:
    ...
    bullets.append(
        "- Use the other tabs to deep–dive into pricing, product mix, geography, timing, and compliance."
    )
    st.markdown("\n".join(bullets))
...
with tab_price:
    ...
    with p_tabs[4]:
        ...
        st.dataframe(p_df[cols].head(300), use_container_width=True)
        st.download_button(
            "Download price-driver subset (CSV)",
            data=p_df[cols].to_csv(index=False).encode("utf-8"),
...
with tab_mix:
    ...
    with m_tabs[3]:
        ...
        st.dataframe(m_df[cols].head(300), use_container_width=True)
        st.download_button(
            "Download product-mix subset (CSV)",
            data=m_df[cols].to_csv(index=False).encode("utf-8"),
...
with tab_segments:
    ...
    with s_tabs[3]:
        ...
        st.dataframe(s_df[cols].head(300), use_container_width=True)
        st.download_button(
            "Download customer-segment subset (CSV)",
            data=s_df[cols].to_csv(index=False).encode("utf-8"),
...
with tab_geo:
    ...
    with g_tabs[0]:
        ...
        top_country = by_c["Country"].iloc[0] if not by_c.empty else "N/A"
        share_top = (
            float(by_c[metric_col].iloc[0] / by_c[metric_col].sum())
            if by_c[metric_col].sum() > 0
            else np.nan
        )
        bullets = []
        if np.isfinite(share_top):
            bullets.append(
                f"- **{top_country}** drives about **{share_top*100:.1f}%** of {metric_label.lower()} in this view."
            )
        bullets.append(
            "- Use the map and heatmap tabs to see how channels shift by market."
        )
        st.markdown("\n".join(bullets))
    ...
    with g_tabs[4]:
        ...
        st.dataframe(g_df[cols].head(300), use_container_width=True)
        st.download_button(
            "Download geography subset (CSV)",
            data=g_df[cols].to_csv(index=False).encode("utf-8"),
...
with tab_timing:
    ...
    if t_df.empty:
        ...
        with t_tabs[0]:
            ...
            st.markdown(
                ...
                **95th Percentile (long tail):** `{p95:.1f}` days  
                ...
                Use the other tabs to see which channels or months drive longer shipping times.
                """
            )
        ...
        with t_tabs[4]:
            ...
            st.dataframe(t_df[cols].head(300), use_container_width=True)
            st.download_button(
                "Download timing subset (CSV)",
                data=t_df[cols].to_csv(index=False).encode("utf-8"),
...
with tab_ownership:
    ...
    with o_tabs[3]:
        ...
        st.dataframe(o_df[cols].head(300), use_container_width=True)
        st.download_button(
            "Download ownership subset (CSV)",
            data=o_df[cols].to_csv(index=False).encode("utf-8"),
...
with tab_seasonality:
    ...
    with se_tabs[3]:
        ...
        st.dataframe(se_df[cols].head(300), use_container_width=True)
        st.download_button(
            "Download seasonality subset (CSV)",
            data=se_df[cols].to_csv(index=False).encode("utf-8"),
...
with tab_compliance:
    ...
    with c_tabs[3]:
        ...
        st.dataframe(c_df[cols].head(300), use_container_width=True)
        st.download_button(
            "Download compliance subset (CSV)",
            data=c_df[cols].to_csv(index=False).encode("utf-8"),
...
with tab_data:
    ...
    st.markdown(
        "This table shows the full dataset after applying filters. "
        "Use the download button to export for deeper analysis."
    )
